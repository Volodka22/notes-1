...
Если пишешь многопоточную программу, есть thread_sanitizer (работает аналогично address_sanitizer). Есть статичные анализаторы -- в кланге это thread safety analyze.


                         
                         
                         
# Lecture 15. Многопоточность 3


Про то, как это устроено внутри

Почему процессор хочет переупорядочивать комманды чтения/записи? Или компилятор это хочет, мы не должны об этом думать.
* Поскольку процессор исполняет спекулятивно, а читать долго, он хочет начать читать за долго до того как эта команда появится, чтобы не ждать потом. Так же с записью, он может ее сделать не в момент команды а позже, когда у него будет возможность сделать это не задерживая общее исполнеение програмы
* То же самое с кэшами, если читаем из кэша а потом из рам, то хочется начать что-то делать с прочитанными быстро из кэшаданными, а не ждать пока дочитается оперативка

В однопоточной программе такое жедание в 99% случаев, так как нам не важно в каком порядке он там что считает/пишет, главное чтобы все корректно сделал. И в многопоточном коде на самом деле тоже, поскольку взаимодействие между потоками происходит не прям чтобы часто, а бОльшую часть времени они ебашут со своими данными.

По большому счету, разрабам процессора приходится выбирать:
* Не упорядочивать -- тогда они замедлят свой процессов
* Могут как-то переупорядочивать, тогда для многопоточных программ придется как-то по-другому специфицировать комманды

Есть пример, где однопоточная программа из-за переупорядочивания чтений/записей начинает работать дольше, это потому что процессор не знает что это однопоточная программа и делает так, чтобы сохранялись все его гарантии.

У интела есть следующие разрешенный переупорядочивания:
* чтение не переупорядочивается с другими чтениями
* записи не переупорядочиваются с предыдущими чтениями
* записи не переупорядочиваются с другими записями (кроме специальный инструкций, нам неважно это)
* чтения могут переупорядочиваться с предыдущими запясями (как раз кейс выше: write; read; где хочется читать начать раньше а писать позже)
* и другие

В плюсовых атомиках под майкрософт load транслируются в обычные mov, а записи транслируются в xchg, это своп, чтобы не иметь возможность переупорядочить записи с чтениями. (у xchg есть неяный префикс log-, а комманды с таким префиксом не могут переупорядочиваться). Можно было бы и наоборот сделать, читать xchg, а писать mov, но чтение более частая операция поэтому ее сделали дешевле.
...
Рассмотрим модель, когда все эти потоки запустили одновременно, хотим узнать что записано в x3, y3, x4, y4

В треде3 интересный кейс x3 == 1, y3 == 0  <=>  thread1 finished before thread2
В треде4 интересный кейс y4 == 1, x4 == 0  <=>  thread2 finished before thread1

Можно подумать, что кейс x3 == 1 && y3 == 0 && x4 == 1 && y4 == 0 невозможен. На х86 это так, на АРМах и некоторых других -- возможен. [pogue]

В х86 написано, что любые две записи верны в прецессированном (каком-то) порядке процессорами, кроме тех, которые сами делали записи. То есть если вы только читаете, то другие чуваки видят вас в одном порядке.

Это кажется дикостью, но в реальности видим, что разные процессоры видят записи в разном порядке.

Представьте, что мы коммитет стандартизации С++. Как нам специфицировать атомики? По сильным гарнтиям, тогда слабые не буду поддерживать; по слабым, тогда производительность упадет?

Есть спецификации к атомикам, их 6, называются memory_order: (см. далее объяснения что есть что)
* relaxed -- Пример (15.2)
* consume -- по силе между relaxed & acquire, может почти везде быть дешовым, но почти везде его заменяют на acquire
* acquire -- Пример (15.1)
* release -- Пример (15.1)
* acq_rel -- гарантии и acquire & release
* seq_cst -- дефолтный, соответствует нашему наивному представлению -- на операциях можно задать модель, какая прошла раньше, какая позже и все видят одинаково

Эти модели подразумевают разные наборы переупорядочиваний, сделано для эффективной поддержки тех архитектур, где любят переупорядочивать.
...
Чтобы это исполнилось корректно, хотим запретить переупорядочивания 2 после 3 и 7 до 6. В терминах С++ atomic это механика acquire-release:
...
1|  ...
2|  value = 42;
3|  value_ready.store(true, memory_order_release);
4|  ...
---------------
thread2:
5|  ...
6|  if (value_ready.load(memory_order_acquire) {
7|      int tmp = value;
8|  }
9|  ...
```
Означает, что **если мы встретили лоад на аквайр и он увидел стор на релиз, то все предыдущие ему записи будут видно после этого лоада для чтения**.

Этого достаточно для реализации мьютекса.
...
main:
.join();
int tmp = event_counter;
```
Тут нам неважно в каком порядке потоки увеличивают счетчик, и неважно когда он внутри потока увеличится. Важно, чтобы в main он прочитался корректно. Это называется **relaxed**. Или если в переменную вне мьютекса пишется из мьютекса, тоже можно позволить писать туда relaxed, поскольку мьютекс гарнтирует порядок.

Главные выводы:
* это полезно иметь в виду, но не надо на это полагаться, только на плюсовые гарантии
* "Такие вещи программировать сложно, не надо такое программировать" (с) Чувака 10 лет назад

**Sharing:**
```
atomic<int> vals;

void thread_proc(std::atomic<int>& val, std::atomic<bool> const& finished) {
    while (!finished) {
        ++val;
    }
}
```
Запустим много потоков на этой функции, как будет меняться значение val от количества потоков? Поскольку потоки не могут одновременно работать с атомарной переменной, то инкрементить ее они будут по очереди. И поскольку чем больше потоков воюет за один участок памяти, тем медленее это все в совокупности работает. Реальный результат зависит от версии ядра (лол). Может ли скедулер повлиять на это? Может, даже если потоков меньше, чем ядер. Если он отправит потоки на одно ядро (гиппертрендинг), то будет выигрыш по скорости потому что у них общий кэш [ыаааа]

**False-sharing:**
```
vector<atomic<int>> vals;
...
А что если теперь в каждый поток передавать свой элемент вектора. Будет тоже плохо, потому что при записи ядро забирает себе всю кэш линию и тут мы будем терять много времени на постоянное переключение этой кэш линии :) Получили тот же sharing но не на одной переменной, а на всей кэш линии.

Возвращаясь к примеру прошлой пары, если сделать очередь не на деке, который лежит сложно в памяти, а на массиве, например, мы тоже будем получать false sharing. Как с этим можно побороться? Можно сделать несколько очередей, и пушить в разные, аналогично с попом. Но эти очереди еще балансировать бы, но это тоже как-то решаемо.

Про аллокаторы памяти и многопоточность. Время, которое поток проводит в malloc, меньше чем время одного кэш-мисса. Нужно чтобы это как-то зорошо работало и с кучей потоков. Сделали n копий однопоточного аллокатора и раздали разным потокам. Есть траблы если выделяется и освобожается в разных потоках, то чуть дольше получается. Но то время, которое вы тратите на передачу данных между потоками, меньше чем время освобождения, поэтому это не то, что оптимизировать нужно. + Они борятся с false sharing, выравнивая хипы по размерам кэш-линий. + Возвращают освобожденую в другом треде память через нулевой хип обратно.





# Lecture 16. Многопоточность 4

Как модифицировать наш сигнал, чтобы можно было работать с ним многопоточно?
* Залочить все операции, используя std::recursive_mutex

    Какие проблемы могут возникнуть?
    - Пусть в сигнале лежит мьютекс m, а во внемшем мире мьютекс m2. Пусть мы залочили m2, вызвали сигнал, он в коллбэке захочет залочить m2, и в тоже время удерживает m. Получили дедлок между m и m2. (такое возникает не только в сигнале, в любом коде с коллбэками) То есть нам нужно знать, какие мьютексы, кто использует, чтобы рассуждать о корректности программы.

* Лочить все операции, но отпускать локи во время коллбэка

    Проблемы:
    - Теряем гарантию, что после того как мы дисконектнули нас вызвать не будут. После того как мы отпустили мьютекс в другом потоке мы можем отписаться, но нас все равно вызовут в первом.
    
* Можно сделать очередь действий для потоков, чтобы эта работа сделалась, когда она освободится (в библиотеки Qt так сделано)

    Это хорошо, когда мы знаем, что объекты живут внутри одного треда, где они и иммитятся.
    
* Закинуть в очередь и подождать, когда исполнится

    Немного черевато дедлоками.
    
* mutex injection

    Инициировать мьютекс из вне, тогда не будет проблемы первого пункта с дедлоком


**Cancellation**

RAII позволяет доказывать корректность програмы
```
struct mytype {
    shared_data data;
    std::thread worker;
...
```
Написав воркер после дата, мы гарантируем, что воркер умрет раньше даты и не останется висеть впустую. Более того, мы гарантируем, что больше одного потока мы не создадим.

Как сказать потоку "давай выходи"?
* atomic<bool>

    Достаточно быстрый способ, но его надо как-то проверять акуратно постоянно. Если мы увидели там true, то что делать?
    Если он крутится в цикле, то это ок, мы можем отменить спокойно.
    А если там много кода, как отмениться в любой точке, чтобы не ждать долго?
    Частый способ: после проверки бросить экспешен наверх.
    ```
    void test_cancell() {
        if (cancelled) {
            throw forced_unwind();
    ...
    - кто-то может поймать. Поэтому советуют не ловить через ...
    - теперь не можем навешить noexcept на функции, который канселятся. В POSIX можно задизэйблить этот маркер на время исполнения тела функции.
    - если кто-то в sleep mode, то есть проблема, что он блокирующий и никак не отреагирует на это. read, write, lock у мьютека, wait у кондишен вэриабла. Тут нужна поддержка ОС. 
    
* Есть вариант со stop_token <-> stop_source

    stop_source проверить легко, но что делать с блокирующими операциями?

Есть ассинхронная cancellation -- это когда канселяться можно на любой комманде. Достаточно опасная штука.

...
**Как ослабили требования на итераторы**: раньше требовали для чтобы у контейнера были begin и end, причем одного типа, для условного вектора это само собой разумеется, но если например есть istram_iterator, то его end это по сути заглушка, которая сообщает что она end, поскольку по инпуту мы бежим только слева направо. А это не слишком эффективно. Теперь разрешили бегин и энд быть разными типами. Такой энд называют sentinel -- он либо итератор, либо специальный тип, умеющий сравниваться на предмет достижения конца диапазона. Даже существует класс unreachable_sentinel.
